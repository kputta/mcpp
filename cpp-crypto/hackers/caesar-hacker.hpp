...
Assumptions:
- text should be from a language, so we apply against a dictionary of the language (english/latin ..)
- given a dictionary, we could come up with best guess. ciphertext word may actually not part of the dictionary, we are looking for key with maximum matches
- we assume words are separated by " " in actual plaintext
- given language we assume range of key is limited to # of chars in that language
