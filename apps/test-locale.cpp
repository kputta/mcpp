...
int main() {
  ...
  std::cout << "byte_string_len: " << plaintext.size()
            << ", wide_string_len: " << conv_.from_bytes(plaintext).size()
            << "\n";
  ...
  std::mbstate_t>>(std::locale()); std::wstring internal =
  L"z\u00df\u6c34\U0001f34c"; // L"zß水🍌"
