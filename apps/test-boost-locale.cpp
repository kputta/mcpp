...
int main(int argc, char** argv) {
  ...
  for (boost::u8_to_u32_iterator<std::string::iterator> it(FLAGS_t.begin()),
       end(FLAGS_t.end());
       it != end; ++it) {
    std::cout << "\"" << *it << "\", ";
  }
  std::cout << std::endl;
  std::cout << boost::locale::conv::from_utf<char>(
                   FLAGS_t, "UTF-8", boost::locale::conv::method_type::stop)
            << std::endl;

  std::string latin1_string = boost::locale::conv::from_utf(FLAGS_t, "UTF-8");
  std::string utf8_string =
      boost::locale::conv::to_utf<char>(latin1_string, "UTF-8");
  std::wstring wide_string =
      boost::locale::conv::to_utf<wchar_t>(latin1_string, "UTF-8");
  std::string utf8_string2 = boost::locale::conv::utf_to_utf<char>(wide_string);

  std::cout << latin1_string << " " << utf8_string << " " << utf8_string2
            << std::endl;
  return 0;
}
